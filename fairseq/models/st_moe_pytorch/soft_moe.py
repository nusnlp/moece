...
from .st_moe_pytorch import MixtureOfExpertsReturn
...
class SoftMoE(Module):
    def __init__(
        ...
        *,
        ...
        norm_class = LayerNorm if use_layernorm else RMSNorm
        self.norm = norm_class(dim)

        self.slot_norm = norm_class(dim)
        self.slot_embeds = nn.Parameter(torch.randn(num_experts, num_slots, dim))

        expert_class = GLUFeedForward if geglu else FeedForward

        experts = default(experts, lambda: [expert_class(dim=dim, mult=expert_mult, dropout=dropout) for _ in range(num_experts)])
    ...
    def forward(self, x, mask = None, add_noise = False, noise_mult = 1.):
        ...
        return MixtureOfExpertsReturn(out, torch.zeros(1, device=x.device), torch.zeros(1, device=x.device), torch.zeros(1, device=x.device))
